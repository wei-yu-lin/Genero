...
DEFINE PDI_tmp CZFP210M_DB.CZFP210M
...
FUNCTION CPSCR602()
    ...
    DEFINE upd_tmp CZFP210M_DB.CZFP210M

    #OPEN FORM CPSCR602_f FROM "CPSCR602"
    #DISPLAY FORM CPSCR602_f

    CALL form_tb.init(TRUE)
    CALL form_tb.setAttribute("_add", FALSE)
    CALL form_tb.setAttribute("_help", FALSE)
    CALL form_tb.getPriv("CPSCR602")
    ...
    DIALOG ATTRIBUTE(UNBUFFERED)
        ...
        BEFORE DIALOG
            ...
            CALL MainDialog.setActionActive("CPSCR602_save", FALSE) --PDI修改確認按鈕disable
            CALL MainDialog.setActionActive("CPSCR602_back", FALSE) --PDI修改取消按鈕disable
        ...
        #以鋼捲編號查詢鋼捲
        ON ACTION CPSCR602_query ATTRIBUTE(TEXT="確認查詢", ACCELERATOR = "F1")
            CALL getPDI(fun_winSelCoil())
            
        ON ACTION CPSCR602_prev -- 前一筆
            LET pt = arr_curr()-1
            IF arr_curr() <= 1 THEN
        ...
        ON ACTION CPSCR602_next -- 下一筆
            ...
            ELSE
                ...
                CALL DIALOG.setCurrentRow("COIL_LIST_tb",pt)
            END IF
        ON ACTION CPSCR602_first -- 第一筆
            CALL dispPDI(dispCoilList.arr[1].COIL_NO)
            CALL DIALOG.setCurrentRow("COIL_LIST_tb",1)
            
        ON ACTION CPSCR602_last -- 最後筆
        ...
        #修改PDI
        ON ACTION CPSCR602_upd ATTRIBUTE(TEXT="修改PDI", ACCELERATOR = "F2")
            CALL setFieldsModSch02(true, MainDialog)
            INITIALIZE upd_tmp.* TO NULL
            CALL MainDialog.setActionActive("CPSCR602_save", TRUE)
            CALL MainDialog.setActionActive("CPSCR602_back", TRUE)
            CALL MainDialog.setActionActive("CPSCR602_upd", FALSE)

        ON ACTION CPSCR602_save -- 修改PDI->確認修改
            ...
            CALL upd_tmp.updCZFP210M()
            LET dispPDI.focus_PDI.* = upd_tmp.*
            LET dispPDI.arr[arr_curr()].* = dispPDI.focus_PDI.*
            CALL setFieldsModSch02(false, MainDialog) 
            
            CALL MainDialog.setActionActive("CPSCR602_upd", TRUE)
            CALL MainDialog.setActionActive("CPSCR602_save", FALSE)
            CALL MainDialog.setActionActive("CPSCR602_back", FALSE)
            
        ON ACTION CPSCR602_back -- 修改PDI->取消修改
            CALL setFieldsModSch02(false, MainDialog)
        ...
        #刪除PDI
        ON ACTION CPSCR602_del ATTRIBUTE(TEXT="刪除PDI", ACCELERATOR = "F3")
            IF fgldialog.fgl_winButton(
                "刪除資料",
                "是否確定刪除"||dispPDI.arr[arr_curr()].COIL_NO||"的PDI?",
                "Lynx",
                "YES" || "|" || "NO",
                "question",
                0) = "YES" THEN
                INITIALIZE upd_tmp.* TO NULL
                LET upd_tmp.COIL_NO = dispPDI.arr[arr_curr()].COIL_NO
                CALL upd_tmp.delCZFP210M()
            END IF
        ON ACTION CPSCR601
            LET next_work = 1
            EXIT DIALOG
...
FUNCTION getPDI(arg_coil_no CHAR(11))
    ...
    SELECT * INTO PDI_tmp.* FROM czfp210m WHERE COIL_NO = arg_coil_no
...
FUNCTION dispPDI(arg_coil_no CHAR(11))
    ...
    LET dispPDI.focus_PDI.COIL_NO = arg_coil_no
    CALL dispPDI.focus_PDI.getCZFP210M()
    ...
    DISPLAY ORDER_THICK, PROD TO ORDER_THICK_SCH02, PROD_SCH02 
END FUNCTION
