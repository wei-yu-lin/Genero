...
#針對每個CPSCAP01_rec欄位用一個STRING定義個欄位顏色屬性
...
FUNCTION (src_list COIL_LIST) extendList(status STRING, num INTEGER)
    ...
    IF status = "init" THEN
        ...
        SELECT BP INTO nextSchdCoil FROM CCAP320M WHERE SCHD_COIL_NO = ' '
        LET idx = 1
    ELSE
        #查詢模式下不啟動查詢擴充
        ...
        SELECT BP INTO nextSchdCoil FROM CCAP320M
    ...
    WHILE TRUE
        TRY
            ...
            SELECT BP INTO nextSchdCoil FROM CCAP320M WHERE SCHD_COIL_NO = nextSchdCoil
            LET idx = idx + 1
