...
IMPORT FGL CPSCAP01_report
IMPORT FGL sys_toolbar

#粗排檔CCAP322W LIST--包含CCAP322W動態空陣列
#細排檔CCAP321W LIST--包含CCAP321W動態空陣列
DEFINE P322W_list CCAP322W_LIST
DEFINE P321W_list CCAP321W_LIST

#CR6的鋼捲篩選條件
DEFINE CAP_COND funCond.funCond_list
DEFINE CAP_COIL funCond.funCoil_list
DEFINE condition_main, condition_sub,coil_main STRING

DEFINE PickUpColor CPSCAP_INC.TypePickUpColor
    
DEFINE schd_seq CPSCAP_INC.SchdSeq_rec
#DEFINE form_tb.priv sys_priv
#DEFINE tbFlag sys_tbDispFlag
...
FUNCTION CPSCAP01()
    ...
    #CALL ui.Window.getCurrent().getForm().loadToolBar("CPSCAP00_toolbar")
    CALL form_tb.init(FALSE)
    CALL form_tb.getPriv("CPSCAP01")
    ...
    DIALOG ATTRIBUTE(UNBUFFERED)
        ...
        BEFORE DIALOG
            ...
            IF form_tb.priv.PQRY != 'N' OR form_tb.priv.PQRY IS NULL THEN
                #載入選取條件到CAP_COND.arr
                CALL CAP_COIL.loadCoilProduct("formonly.coil_cb")
                CALL CAP_COND.loadCondition("formonly.condition_cb")                
                #載入暫存檔到當前工作環境
        ...
        #選取粗排到細排
        ON ACTION CAPSCH01_F1 ATTRIBUTE(TEXT="F1 選入待排", ACCELERATOR = "F1")
        ...
        #從細排移除
        ON ACTION CAPSCH01_F2 ATTRIBUTE(TEXT="F2 取消待排", ACCELERATOR = "F2")
            #初始化參數
        ...
        #從清空細排
        ON ACTION CAPSCH01_F3 ATTRIBUTE(TEXT="F3 清空待排", ACCELERATOR = "F3")
            #細排陣列清空
            CALL P321W_list.arr.clear()
            #細排暫存檔清空
            #CALL CCAP321W_DB.delP321W_DB('*')
            ...
            CALL setPickUpColor(ui.DIALOG.getCurrent())
        #修改製程
        ON ACTION CAPSCH01_F4 ATTRIBUTE(TEXT="F4 修改製程", ACCELERATOR = "F4")
            
            #依細排設製"修改製程"欄位
            CALL modify_prod_dm()
            
        #送出排程並產生PDI
        ON ACTION CAPSCH01_F5 ATTRIBUTE(TEXT="F5 送出排程", ACCELERATOR = "F5")
            IF fun_winSendSchd() == 1 THEN
                CALL fgldialog.fgl_winMessage("送出排程","排程送出完成","sucess")
            END IF
        #列印報表    
        ON ACTION CAPSCH01_F6 ATTRIBUTE(TEXT="F6 列印報表", ACCELERATOR = "F6")
            IF P321W_list.arr.getLength() < 1 THEN
                CALL fgldialog.fgl_winMessage("待生產排程報表無資料","\"待排程鋼捲\"列表為空，請將鋼捲選入待排!", "error")
            ELSE
                CALL CPSCAP01_report.GenReport(P321W_list.arr)
            END IF
        ON ACTION CPSCAP02
            LET next_work = 2
            EXIT DIALOG
    ...
    DISPLAY ARRAY P322W_list.arr TO P322W_r.* 
        ...
        BEFORE ROW
            ...
            IF form_tb.priv.F1 != 'N' OR form_tb.priv.F1 IS NULL THEN
                CALL DIALOG.setActionActive("CAPSCH01_F1", TRUE)
            END IF
            #取消排程按鈕關閉
            CALL DIALOG.setActionActive("CAPSCH01_F2", FALSE)
            #修改製程按鈕開啟
            CALL DIALOG.setActionActive("CAPSCH01_F4", FALSE)
...
DIALOG P321W_dm()
    ...
    DEFINE drag_321W_arr DYNAMIC ARRAY OF CCAP321W_DB.CCAP321W
    ...
    DISPLAY ARRAY P321W_list.arr TO P321W_r.*
        ...
        BEFORE ROW
            #選入排程按鈕關閉
            CALL DIALOG.setActionActive("CAPSCH01_F1", FALSE)
            IF form_tb.priv.F2 != 'N' OR form_tb.priv.F2 IS NULL THEN
                #取消排程按鈕開啟
                CALL DIALOG.setActionActive("CAPSCH01_F2", TRUE)
            END IF
            IF form_tb.priv.F4 != 'N' OR form_tb.priv.F4 IS NULL THEN
                #修改製程按鈕開啟
                CALL DIALOG.setActionActive("CAPSCH01_F4", TRUE)
            END IF
...
DIALOG coil_dm()
    ...
    INPUT coil_main FROM coil_cb
        BEFORE FIELD coil_cb
...
DIALOG condition_dm()
    ...
    INPUT condition_main FROM condition_cb
        ...
        ON CHANGE condition_cb
            ...
            IF condition_main = "RE_ACID"  THEN

                CALL ui.Window.getCurrent().findNode("FormField","formonly.select_pb").setAttribute("hidden","0")
                #粗排初始化，避免殘留
                CALL P322W_list.delByCoilNo("*")
                LET P322W_list = CAP_COND.arr[condition_main].etlToP322W(condition_sub,CAP_COIL.arr[coil_main])
                CALL P322W_list.set()
    ...
    INPUT condition_sub FROM condition_sub_cb
        ...
        ON CHANGE condition_sub_cb
            ...
            CASE condition_main
                ...
                WHEN "P_PREV_STATION"
                    #抓取符合條件的鋼捲
                    LET P322W_list = CAP_COND.arr[condition_sub].etlToP322W(condition_sub,CAP_COIL.arr[coil_main])
                OTHERWISE
                    #抓取符合條件的鋼捲
                    LET P322W_list = CAP_COND.arr[condition_main].etlToP322W(condition_sub,CAP_COIL.arr[coil_main])
            END CASE
...
FUNCTION modify_prod_dm()
    ...
    DEFINE mod_prod CPSCAP_INC.ModProd_rec
...
FUNCTION fun_winSendSchd()
    ...
    DEFINE ins_tmp CCAP215M_DB.CCAP215M
    ...
    IF NOT INT_FLAG THEN
        ...
        FOR i = 1 TO schd_seq.coil_num
            #加入PDI Trigger
            LET ins_tmp.COIL_NUMBER = P321W_list.arr[1].COIL_NO
            CALL ins_tmp.insCCAP215M()
...
FUNCTION SetUpSubCond_cb()
    ...
    DEFINE conditionMain_list ui.ComboBox

    LET conditionMain_list = ui.ComboBox.forName("formonly.condition_cb")
    LET cb_list = ui.ComboBox.forName("formonly.condition_sub_cb")
    -- CALL conditionMain_list.clear()
    CALL cb_list.clear()
...
#-------------------------------------------------------
FUNCTION CAPSCH01_init()
    CALL init_CCAP322W_DB()
    CALL init_CCAP322W_LIST()
